...
fn bench_observe_warm_a11y_only(c: &mut Criterion) {
    let assembler = ObservationAssembler::new();
    let include = ObserveInclude::default();
    let input = synthetic_notepad_input(None);
    c.bench_function("observe_warm_a11y_only", |bench| {
        bench.iter(|| {
...
criterion_group!(benches, bench_observe_warm_a11y_only);
