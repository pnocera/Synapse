...
fn panic_hook_timeout_keeps_release_all_in_queue_and_preserves_previous_hook() {
    ...
    println!("source_of_truth=panic_hook_log edge=timeout raw={log_output:?}");
    ...
    println!(
        "source_of_truth=panic_hook_queue edge=timeout after_queued_before_drain:1 after_drained_action:{action_label} previous_count:{} panic_caught:{} log_line={log_line}",
        previous_count.load(Ordering::SeqCst),
        result.is_err()
    );
}
