...
fn panic_hook_releases_before_prior_hook_and_installs_once() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "source_of_truth=panic_hook_queue edge=happy before=release_count:{} previous_count:{} prior_observed_release:{} install_calls:0",
    ...
    println!(
        "source_of_truth=panic_hook_queue edge=happy after_event={first_event} after=release_count:{} previous_count:{} prior_observed_release:{} panic_caught:{}",
        release_count.load(Ordering::SeqCst),
        previous_count.load(Ordering::SeqCst),
        previous_observed_release_count.load(Ordering::SeqCst),
        first_result.is_err()
    ...
    println!(
        "source_of_truth=panic_hook_queue edge=idempotent before=release_count:{} previous_count:{} prior_observed_release:{} install_calls:2",
        release_count.load(Ordering::SeqCst),
        previous_count.load(Ordering::SeqCst),
        previous_observed_release_count.load(Ordering::SeqCst)
    );
    ...
    println!(
        "source_of_truth=panic_hook_queue edge=idempotent after_event={second_event} after=release_count:{} previous_count:{} prior_observed_release:{} release_delta:1 previous_delta:1 panic_caught:{}",
        release_count.load(Ordering::SeqCst),
        previous_count.load(Ordering::SeqCst),
        previous_observed_release_count.load(Ordering::SeqCst),
