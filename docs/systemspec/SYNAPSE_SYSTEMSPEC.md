...
1. [01_system_overview.md](#file-01) — architecture map, tech stack, 67-tool inventory, error hierarchy
...
5. [05_core_types_and_errors.md](#file-05) — `synapse-core` wire types + 105 error codes
...
All 67 live tools live in `crates/synapse-mcp/src/server.rs` (declared via `#[tool_router]`). Grouped by milestone:
...
### 4.6 EverQuest live evaluation/world model (15 tools)
...
All 105 codes are `pub const &'static str` in `crates/synapse-core/src/error_codes.rs`. Mapped from each subsystem's `thiserror` enum's `.code()` method. Categories with line ranges (see [01_system_overview.md §8](#file-01) for the table).
...
M5 profile-registry security adds `PROFILE_TRUST_VERIFICATION_FAILED` for
...
Implements `rmcp::ServerHandler` via `#[tool_handler(router = self.tool_router)]`. The 67 tools are declared on the same struct under milestone and EverQuest `#[tool_router]` impls; `#[tool(description = "...")]` annotations produce JSON-Schema entries for `tools/list` automatically.
...
- `act_combo`, `act_run_shell`, `act_launch` — three M4 tools that bring the live MCP tool count from 30 -> 33; #499 adds `act_keymap` for profile keymap aliases; M5 profile-registry/audit work adds `profile_quality_refresh`, six `profile_authoring_*` candidate tools, eight `profile_registry_*` tools including the report inspector and rollback, `audit_intelligence_query`, `audit_export_consent_set`, and `audit_export_bundle`; #508/#524/#510/#525/#526/#527/#528/#514/#511/#512/#513/#531 add the EverQuest `/loc`, visible chat-input state, current-state, map-sensor, outcome, route, memory, planner-guard, DynamicJEPA domain normalization, linked trajectory, approved-prefix world-model rows/readback, and action-prior tools, bringing the live surface to 67.
...
baseline and now records the approved 67-tool live surface after M4/M5,
...
| 8 | `act_type` | M2 | live | |
| 9 | `act_press` | M2 | live | |
| 10 | `act_keymap` | M2/#499 | live | resolves active-profile keymap aliases and audits alias plus resolved key/chord |
| 11 | `act_aim` | M2 | live | Element / Track targets return `ACTION_BACKEND_UNAVAILABLE` |
| 12 | `act_drag` | M2 | live | |
| 13 | `act_scroll` | M2 | live | |
| 14 | `act_pad` | M2 | live | |
| 15 | `act_clipboard` | M2 | live | |
| 16 | `release_all` | M2 | live | |
| 17 | `subscribe` | M3 | live | `buffer_size` pinned at 4096 |
| 18 | `subscribe_cancel` | M3 | live | |
| 19 | `reflex_register` | M3 | live | |
| 20 | `reflex_cancel` | M3 | live | |
...
| 28 | `storage_inspect` | M3 (operator) | live | per-CF row+byte size readback |
| 29 | `storage_put_probe_rows` | M3 (operator) | live | manual storage write/readback support tool |
| 30 | `storage_gc_once` | M3/M5 (operator) | live | synchronous GC pass; `AUDIT_RETENTION` mode writes #463 retention/dedupe/backfill report rows |
| 31 | `storage_pressure_sample` | M3 (operator) | live | synthetic disk-pressure trigger |
| — | `read_hud` | (deferred to M4) | not live | standalone tool deferred; profile HUD extraction is live through `observe` |
| 32 | `act_combo` | M4 | live | one-shot timed action sequence |
| 33 | `act_run_shell` | M4 (gated) | live | allowlisted local shell command |
| 34 | `act_launch` | M4 (gated) | live | allowlisted local process launch |
| 35 | `profile_authoring_generate` | M5 (registry/audit) | live | writes a profile-authoring candidate row from replay/audit evidence |
| 36 | `profile_authoring_list` | M5 (registry/audit) | live | lists local authoring candidate rows |
| 37 | `profile_authoring_inspect` | M5 (registry/audit) | live | reads one candidate row |
| 38 | `profile_authoring_accept` | M5 (registry/audit) | live | marks a candidate accepted without activation |
| 39 | `profile_authoring_reject` | M5 (registry/audit) | live | marks a candidate rejected |
| 40 | `profile_authoring_export` | M5 (registry/audit) | live | exports one candidate bundle |
| 41 | `profile_quality_refresh` | M5 (registry/audit) | live | writes `CF_PROFILES` quality snapshot from `CF_ACTION_LOG` |
| 42 | `profile_registry_search` | M5 (registry/audit) | live | searches `CF_PROFILES` registry rows |
| 43 | `profile_registry_inspect` | M5 (registry/audit) | live | reads one `CF_PROFILES`/`CF_KV` registry row |
| 44 | `profile_registry_report` | M5 (registry/audit) | live | reports registry, quality, audit, consent, quarantine, and SoT pointers |
| 45 | `profile_registry_install` | M5 (registry/audit) | live | validates package manifest/profile TOML and writes registry rows |
| 46 | `profile_registry_disable` | M5 (registry/audit) | live | disables or removes an installed registry row |
| 47 | `profile_registry_export` | M5 (registry/audit) | live | exports local registry or contribution bundle |
| 48 | `profile_registry_import` | M5 (registry/audit) | live | imports validated registry/contribution bundle |
| 49 | `profile_registry_rollback` | M5 (registry/audit) | live | restores installed profile to a prior trusted package |
...
| 66 | `everquest_action_prior_record` | M4/M5 (EverQuest) | live | prediction/outcome sample rows |
| 67 | `everquest_action_prior_scorecard` | M4/M5 (EverQuest) | live | floor-not-ceiling competence scorecard rows |
| — | `describe` | M5 (VLM) | not live | Florence-2 |

Live count in `crates/synapse-mcp/src/server.rs`: **67** (M1: 6,
...
diagnostics, plus M4 `act_combo`/`act_run_shell`/`act_launch`, plus 15
...
- `m3_tools_list.rs` / `m4_tools_list.rs` — `tools/list` exposes the current 67-tool surface, including #499 `act_keymap`, M5 profile-registry/audit tools, #462 `profile_authoring_*`, #468 `profile_registry_report`, `profile_registry_rollback`, #460 `audit_export_*`, and the EverQuest world-model tools through `everquest_chat_input_state`, `everquest_planner_guard`, `everquest_domain_normalize`, `everquest_trajectory_record`, `everquest_world_model_record`, `everquest_world_model_inspect`, and `everquest_action_prior_scorecard`
...
| Total Rust source files (excluding tests/benches) | **156** |
| Total Rust integration-test files | 76 |
| Total Rust bench files | 13 |
| MCP tools registered in `server.rs` | **67** (M1/M2/M3/M4/M5 plus EverQuest runtime tools including `/loc`, chat-input state, current-state, map sensor, outcome ingest, memory, planner guard, route plan, DynamicJEPA domain normalize, trajectory record, approved-prefix world-model record/inspect, and action-prior scorecard) |
| MCP tools approved by `05_mcp_tool_surface.md` (agent surface cap) | 67 |
