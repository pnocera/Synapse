...
|---|---|---|---|
| `synapse-mcp` | `crates/synapse-mcp` | **67 MCP tools live** (M3 baseline plus #499 `act_keymap`, M4 `act_combo`/`act_run_shell`/`act_launch`, M5 profile registry/audit/authoring/export tools, and EverQuest `/loc`, chat-input state, current-state, map-sensor, outcome, route, guard, memory, DynamicJEPA domain normalization, linked trajectory rows, approved-prefix world-model rows/readback, and action-prior tools); streamable HTTP + SSE transport live on loopback with bearer auth + Origin/Host check + `Mcp-Session-Id` lifecycle; M3/M5 permission gating wired; profile, storage, reflex, authoring, registry, audit-intelligence, audit-export, and EverQuest world-state surfaces live | Continue M5 registry/audit moat work |
