...
|---|---|---|---|
| `synapse-mcp` | `crates/synapse-mcp` | **34 MCP tools live** (M3 baseline plus M4 `act_combo`/`act_run_shell`/`act_launch` and M5 `profile_quality_refresh`); streamable HTTP + SSE transport live on loopback with bearer auth + Origin/Host check + `Mcp-Session-Id` lifecycle; M3/M5 permission gating wired; profile, storage, reflex, and registry-quality health surfaced | Continue M5 registry/audit moat work |
